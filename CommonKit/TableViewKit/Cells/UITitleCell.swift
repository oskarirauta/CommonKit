...
extension UITableViewCell {
    ...
    open class TitleCell: UITableViewCell.Extended, CellTitle {
        ...
        open var italic: Bool {
            ...
            set {
                self.titleLabel.font = self.titleFont.fontDescriptor.symbolicTraits.contains(.traitItalic) ? UIFont.italicSystemFont(ofSize: self.titleLabel.font.pointSize) : UIFont.boldSystemFont(ofSize: self.titleLabel.font.pointSize)
            }
        }
        ...
        override open func cellProperties() {
            ...
            self.iconColor = nil
            self.titleLabel.iconWidthSource = .manual( iphoneCompatible ? 19.5 : 39.0 )
            self.titleFont = UIFont.boldSystemFont(ofSize: iphoneCompatible ? 13.5 : 27.0)
        ...
        override open func setupConstraints() {
            ...
            self.titleLabel.bottomAnchor.constraint(lessThanOrEqualTo: self.contentView.bottomAnchor, constant: ( iphoneCompatible ? -8.0 : -16.0 )).isActive = true
        }
           
        open override func secondaryConstraints() {
               
            self.cellConstraints["titleLabel-trailing"] = self.titleLabel.trailingAnchor.constraint(equalTo: self.contentView.trailingAnchor, constant: ( iphoneCompatible ? -8.0 : -16.0 ))
        }
        ...
        open func internalSetSelected(_ selected: Bool, animated: Bool) {
            ...
            if ( self.tableView?.isEditing ?? false ) { self.controllerProtocol?.endEditing() }
