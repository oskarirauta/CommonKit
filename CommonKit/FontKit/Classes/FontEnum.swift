...
public protocol FontEnum: FontType, EnumCollection, RawRepresentable where RawValue == String {
...
extension FontEnum {
    ...
    public init?(rawValue: Self.RawValue) {
        ...
        guard let font = Self.allValues.first(where: { $0.prefix == lname }) else { return nil }
        self = font
    }
    ...
    public init?(fontNo: Int) {
        guard let font = Self.allValues.first(where: { $0.fontNo == fontNo }) else { return nil }
        self = font
    }
    
    public func icon(named: String) -> IconType? {
        guard let index: Int = Self.allValues.sorted(by: { $0.fontNo < $1.fontNo }).index(of: self) else { return nil }
