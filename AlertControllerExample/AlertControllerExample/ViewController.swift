...
class ViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    weak var secureTextAlertAction: AlertAction?
    var customAlertController: AlertController!
    weak var textField1: UITextField?
    weak var textField2: UITextField?
    weak var customAlertAction: AlertAction?
    var tableView: UITableView = UITableView(frame: .zero, style: .grouped)
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationController?.navigationBar.isTranslucent = false
        self.title = "AlertController Example"
    ...
    // MARK: AlertControllerStyleAlert Style Alerts
    ...
    func showOkayCancelAlert(_: IndexPath) {
        ...
        let alertCotroller = AlertController(title: title, message: message, preferredStyle: .alert)
    ...
    func showOtherAlert(_: IndexPath) {
        ...
        let otherButtonOneAction = AlertAction(title: otherButtonTitleOne, style: .default) { action in
            NSLog("The \"Other\" alert's other button one action occured.")
        }
        
        let otherButtonTwoAction = AlertAction(title: otherButtonTitleTwo, style: .default) { action in
            NSLog("The \"Other\" alert's other button two action occured.")
        }
        
        let destructiveButtonAction = AlertAction(title: destructiveButtonTitle, style: .destructive) { action in
    ...
    func showTextEntryAlert(_: IndexPath) {
        ...
        present(alertController, animated: true, completion: {
            let _ = alertController.textFields?.first?.becomeFirstResponder()
        })
    }
    ...
    func showCustomAlert(_: IndexPath) {
        ...
        customAlertController = AlertController(title: title, message: message, preferredStyle: .alert)
        
        // OverlayView
        customAlertController.overlayColor = UIColor(red:235/255, green:245/255, blue:255/255, alpha:0.7)
        // AlertView
        customAlertController.contentViewBgColor = UIColor(red:44/255, green:62/255, blue:80/255, alpha:1)
        // Title
        ...
        present(customAlertController, animated: true, completion: {
            let _ = self.customAlertController.textFields?.first?.becomeFirstResponder()
        })
    }
    
    // MARK: AlertControllerStyleActionSheet Style Alerts
    ...
    func showCustomActionSheet(_ selectedIndexPath: IndexPath) {
        ...
        // AlertView
        alertController.contentViewBgColor = UIColor(red:44/255, green:62/255, blue:80/255, alpha:1)
        // Title
    ...
    @objc func handleTextFieldTextDidChangeNotification(_ notification: Notification) {
        ...
        secureTextAlertAction!.enabled = ( textField.text?.count ?? 0 ) >= 5
    }
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if (textField === textField1) {
            ...
            customAlertAction!.handler = self.customAlertAction?.handler
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return section == 0 ? "Alert style" : "Action sheet style"
    }
    ...
    let titles: [[String]] = [
        ["Simple", "Okay/Cancel", "Other", "Text Entry", "Secure Text Entry", "Custom"],
        ["Okay/Cancel", "Other", "Custom"]
    ]
